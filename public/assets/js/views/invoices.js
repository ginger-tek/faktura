...
export default {
  template: `<div>
    ...
    <auto-table :data="invoices" :columns="invoiceColumns" :bordered="true" :filter="filter" class="nowrap">
      ...
      <template #labor_amount="{ labor_amount }">{{ toCurrency(labor_amount) }}</template>
      <template #expense_amount="{ expense_amount }">{{ toCurrency(expense_amount) }}</template>
      <template #total_amount="{ total_amount }">{{ toCurrency(total_amount) }}</template>
      <template #paid_amount="{ paid_amount }">{{ toCurrency(paid_amount) }}</template>
  ...
  setup() {
    ...
    return {
      ...
      fetchInvoices, fetchClients, createInvoice, toCurrency, toDate
    }
  }
}