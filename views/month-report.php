<div class="flex stack align-center spaced mb-1">
  <h2 class="m-0">Monthly Report</h2>
  <form method="GET" class="flex">
    <button type="button" class="info"
      onclick="this.nextElementSibling.value = '<?= $previous_month ?>'; this.form.submit()">Prev.</button>
    <input style="width:150px" type="month" name="filter_month" value="<?= $filter_month ?? date('Y-m') ?>"
      onchange="this.form.submit()">
    <button type="button" class="info"
      onclick="this.previousElementSibling.value = '<?= $next_month ?>'; this.form.submit()">Next</button>
    <?php if ($filter_month != date('Y-m')): ?>
      <button type="button" onclick="window.location.href='/reports/month'" class="info">Reset</button>
    <?php endif; ?>
  </form>
</div>
...
<div class="chart-wrap">
  <div class="chart">
    ...
    <?php foreach ($chart['days'] as $slot): ?>
      <div class="slot <?= $slot['date']->format('Y-m-d') === date('Y-m-d') ? 'today' : '' ?>"
        title="<?= $slot['date']->format('F j') . ' Income: ' . \App\Utils::to_currency($slot['paid_income']) ?>">
        <div class="bars">
        ...
        <div class="label"><?= $slot['date']->format('j') ?></div>
      </div>
    <?php endforeach; ?>
  </div>
</div>
